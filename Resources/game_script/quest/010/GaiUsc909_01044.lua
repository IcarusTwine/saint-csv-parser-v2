(function()
  print("GaiUsc909 loaded")
  function GaiUsc909.OnScene00000(A0_0, A1_1, A2_2)
    A2_2:LookAt(A1_1)
    if A0_0:QuestOffer(A2_2, A1_1) then
      return 1
    else
      return 0
    end
  end
  function GaiUsc909.OnScene00001(A0_3, A1_4, A2_5)
    A2_5:TurnTo(A1_4, false)
    A2_5:Talk(A1_4, A0_3, A0_3.TEXT_GAIUSC909_01044_MEMEZOFU_000_000, false)
    A2_5:Talk(A1_4, A0_3, A0_3.TEXT_GAIUSC909_01044_MEMEZOFU_000_001, true)
    A0_3:QuestAccepted()
  end
  function GaiUsc909.OnScene00002(A0_6, A1_7, A2_8)
    A2_8:Talk(A1_7, A0_6, A0_6.TEXT_GAIUSC909_01044_ENPC1_000_010, true)
    A0_6:Inventory(true)
  end
  function GaiUsc909.OnScene00003(A0_9, A1_10, A2_11)
    A0_9:Wait(15)
    A2_11:PlayVfx(A0_9.VFX_REACTION)
    A2_11:LookAt(A1_10)
    A2_11:Talk(A1_10, A0_9, A0_9.TEXT_GAIUSC909_01044_ENPC1_000_015, true)
    A2_11:LookAt()
    A2_11:TurnTo(-45, false)
    A2_11:WaitForTurn()
    A0_9:Wait(25)
    A2_11:WalkOut(0, 5, A0_9.MOVE_WALK)
    A0_9:Wait(0)
    A2_11:Transparency(A0_9.TRANS_TYPE_FADE_OUT, 30)
    A2_11:WaitForTransparency()
  end
  function GaiUsc909.OnScene00004(A0_12, A1_13, A2_14)
    A2_14:Talk(A1_13, A0_12, A0_12.TEXT_GAIUSC909_01044_ENPC2_000_020, true)
    A0_12:Inventory(true)
  end
  function GaiUsc909.OnScene00005(A0_15, A1_16, A2_17)
    A0_15:Wait(15)
    A2_17:PlayVfx(A0_15.VFX_REACTION)
    A2_17:LookAt(A1_16)
    A2_17:Talk(A1_16, A0_15, A0_15.TEXT_GAIUSC909_01044_ENPC2_000_025, true)
    A2_17:LookAt()
    A2_17:TurnTo(0, false)
    A2_17:WaitForTurn()
    A0_15:Wait(65)
    A2_17:WalkOut(-45, 5, A0_15.MOVE_WALK)
    A0_15:Wait(0)
    A2_17:Transparency(A0_15.TRANS_TYPE_FADE_OUT, 30)
    A2_17:WaitForTransparency()
  end
  function GaiUsc909.OnScene00006(A0_18, A1_19, A2_20)
    A2_20:Talk(A1_19, A0_18, A0_18.TEXT_GAIUSC909_01044_ENPC3_000_010, true)
    A0_18:Inventory(true)
  end
  function GaiUsc909.OnScene00007(A0_21, A1_22, A2_23)
    A0_21:Wait(15)
    A2_23:PlayVfx(A0_21.VFX_REACTION)
    A2_23:TurnTo(180, false)
    A2_23:WaitForTurn()
    A0_21:Wait(30)
    A2_23:LookAt(A1_22)
    A2_23:Talk(A1_22, A0_21, A0_21.TEXT_GAIUSC909_01044_ENPC3_000_030, true)
    A2_23:LookAt()
    A2_23:WalkOut(120, 5, A0_21.MOVE_WALK)
    A0_21:Wait(0)
    A2_23:Transparency(A0_21.TRANS_TYPE_FADE_OUT, 30)
    A2_23:WaitForTransparency()
  end
  function GaiUsc909.OnScene00008(A0_24, A1_25, A2_26)
    A2_26:Talk(A1_25, A0_24, A0_24.TEXT_GAIUSC909_01044_ENPC4_000_010, true)
    A0_24:Inventory(true)
  end
  function GaiUsc909.OnScene00009(A0_27, A1_28, A2_29)
    A0_27:Wait(15)
    A2_29:PlayVfx(A0_27.VFX_REACTION)
    A2_29:TurnTo(0, false)
    A2_29:WaitForTurn()
    A0_27:Wait(30)
    A2_29:LookAt(A1_28)
    A2_29:Talk(A1_28, A0_27, A0_27.TEXT_GAIUSC909_01044_ENPC4_000_040, true)
    A2_29:LookAt()
    A2_29:WalkOut(0, 5, A0_27.MOVE_WALK)
    A0_27:Wait(0)
    A2_29:Transparency(A0_27.TRANS_TYPE_FADE_OUT, 30)
    A2_29:WaitForTransparency()
  end
  function GaiUsc909.OnScene00010(A0_30, A1_31, A2_32)
    local L3_33, L4_34
    L4_34 = A2_32
    L3_33 = A2_32.TurnTo
    L3_33(L4_34, A1_31, false)
    L4_34 = A2_32
    L3_33 = A2_32.WaitForTurn
    L3_33(L4_34)
    L4_34 = A2_32
    L3_33 = A2_32.PlayActionTimeline
    L3_33(L4_34, A0_30.ACTION_TIMELINE_EMOTE_JOY)
    L4_34 = A2_32
    L3_33 = A2_32.Talk
    L3_33(L4_34, A1_31, A0_30, A0_30.TEXT_GAIUSC909_01044_MEMEZOFU_000_050, false)
    L4_34 = A2_32
    L3_33 = A2_32.PlayActionTimeline
    L3_33(L4_34, A0_30.ACTION_TIMELINE_EVENT_TALK2)
    L4_34 = A2_32
    L3_33 = A2_32.Talk
    L3_33(L4_34, A1_31, A0_30, A0_30.TEXT_GAIUSC909_01044_MEMEZOFU_000_051, false)
    L4_34 = A2_32
    L3_33 = A2_32.Talk
    L3_33(L4_34, A1_31, A0_30, A0_30.TEXT_GAIUSC909_01044_MEMEZOFU_000_052, true)
    L4_34 = A0_30
    L3_33 = A0_30.QuestReward
    L4_34 = L3_33(L4_34, A2_32, A1_31)
    if L3_33 then
      A0_30:QuestCompleted()
    end
    return L3_33, L4_34
  end
  function GaiUsc909.GetEventItems(A0_35, A1_36)
    local L2_37
    L2_37 = A0_35.GetQuestId
    L2_37 = L2_37(A0_35)
    if A1_36:GetQuestSequence(L2_37) == A0_35.SEQ_0 then
      return A0_35.ITEM0, A1_36:GetQuestUI8BH(L2_37), false
    elseif A1_36:GetQuestSequence(L2_37) == A0_35.SEQ_1 then
      return A0_35.ITEM0, A1_36:GetQuestUI8CL(L2_37), true
    else
    end
  end
  function GaiUsc909.IsTodoChecked(A0_38, A1_39, A2_40)
    local L3_41
    L3_41 = A0_38.GetQuestId
    L3_41 = L3_41(A0_38)
    if A1_39:GetQuestSequence(L3_41) == A0_38.SEQ_0 then
      return false
    end
    if A2_40 == 0 then
      return A1_39:GetQuestUI8AH(L3_41) >= 4
    elseif A2_40 == 1 then
      return false
    end
  end
end)()
;(function()
  local L0_42, L1_43
  L0_42 = GaiUsc909
  L0_42.SCRIPT_VERSION = 1
  L0_42 = GaiUsc909
  function L1_43(A0_44)
    local L1_45
  end
  L0_42.OnInitialize = L1_43
  L0_42 = GaiUsc909
  function L1_43(A0_46, A1_47, A2_48, A3_49, A4_50)
    local L5_51
    L5_51 = A0_46.GetQuestId
    L5_51 = L5_51(A0_46)
    if A1_47:GetQuestSequence(L5_51) == A0_46.SEQ_1 then
      if A3_49 == A0_46.ACTOR1 then
        if 1 <= A1_47:GetQuestUI8AL(L5_51) then
          return false
        end
        return A1_47:GetQuestBitFlag8(L5_51, 1) == false
      elseif A3_49 == A0_46.ACTOR2 then
        if 1 <= A1_47:GetQuestUI8BH(L5_51) then
          return false
        end
        return A1_47:GetQuestBitFlag8(L5_51, 2) == false
      elseif A3_49 == A0_46.ACTOR3 then
        if 1 <= A1_47:GetQuestUI8BL(L5_51) then
          return false
        end
        return A1_47:GetQuestBitFlag8(L5_51, 3) == false
      elseif A3_49 == A0_46.ACTOR4 then
        if 1 <= A1_47:GetQuestUI8CH(L5_51) then
          return false
        end
        return A1_47:GetQuestBitFlag8(L5_51, 4) == false
      end
    end
    return false
  end
  L0_42.IsAcceptEvent = L1_43
  L0_42 = GaiUsc909
  function L1_43(A0_52, A1_53, A2_54, A3_55, A4_56)
    local L5_57
    L5_57 = A0_52.GetQuestId
    L5_57 = L5_57(A0_52)
    if A1_53:GetQuestSequence(L5_57) == A0_52.SEQ_1 then
      if A3_55 == A0_52.ACTOR1 then
        if 1 <= A1_53:GetQuestUI8AL(L5_57) then
          return false
        end
        return A1_53:GetQuestBitFlag8(L5_57, 1) == false
      elseif A3_55 == A0_52.ACTOR2 then
        if 1 <= A1_53:GetQuestUI8BH(L5_57) then
          return false
        end
        return A1_53:GetQuestBitFlag8(L5_57, 2) == false
      elseif A3_55 == A0_52.ACTOR3 then
        if 1 <= A1_53:GetQuestUI8BL(L5_57) then
          return false
        end
        return A1_53:GetQuestBitFlag8(L5_57, 3) == false
      elseif A3_55 == A0_52.ACTOR4 then
        if 1 <= A1_53:GetQuestUI8CH(L5_57) then
          return false
        end
        return A1_53:GetQuestBitFlag8(L5_57, 4) == false
      end
    end
    return false
  end
  L0_42.IsAnnounce = L1_43
  L0_42 = GaiUsc909
  function L1_43(A0_58, A1_59, A2_60, A3_61)
    local L4_62
    L4_62 = A0_58.GetQuestId
    L4_62 = L4_62(A0_58)
    if A1_59:GetQuestSequence(L4_62) == A0_58.SEQ_1 then
      if A2_60:GetBaseId() == A0_58.ACTOR1 then
        if A3_61 == A0_58.ITEM0 then
          return true
        end
      elseif A2_60:GetBaseId() == A0_58.ACTOR2 then
        if A3_61 == A0_58.ITEM0 then
          return true
        end
      elseif A2_60:GetBaseId() == A0_58.ACTOR3 then
        if A3_61 == A0_58.ITEM0 then
          return true
        end
      elseif A2_60:GetBaseId() == A0_58.ACTOR4 and A3_61 == A0_58.ITEM0 then
        return true
      end
    end
    return false
  end
  L0_42.IsEventItemUsable = L1_43
  L0_42 = GaiUsc909
  function L1_43(A0_63, A1_64, A2_65)
    local L3_66
    L3_66 = A0_63.GetQuestId
    L3_66 = L3_66(A0_63)
    if A1_64:GetQuestSequence(L3_66) == A0_63.SEQ_0 then
      return 0, 0
    end
    if A2_65 == 0 then
      return A1_64:GetQuestUI8AH(L3_66), 4
    elseif A2_65 == 1 then
      return A1_64:GetQuestUI8AL(L3_66), 0
    end
  end
  L0_42.GetTodoArgs = L1_43
  L0_42 = GaiUsc909
  function L1_43(A0_67, A1_68, A2_69)
    local L3_70
    L3_70 = A0_67.GetQuestId
    L3_70 = L3_70(A0_67)
    if A1_68:GetQuestSequence(L3_70) == A0_67.SEQ_1 then
    elseif A1_68:GetQuestSequence(L3_70) == A0_67.SEQ_FINISH then
    end
    return A0_67:IsBattleNpcTriggerOwner(A1_68, A2_69, false), false
  end
  L0_42.GetGimmickState = L1_43
end)()
