(function()
  print("ClsAcn149 loaded")
  function ClsAcn149.OnScene00000(A0_0, A1_1, A2_2)
    if A0_0:QuestOffer(A2_2, A1_1) then
      return 1
    else
      return 0
    end
  end
  function ClsAcn149.OnScene00001(A0_3, A1_4, A2_5)
    A2_5:TurnTo(A1_4, false)
    A2_5:Talk(A1_4, A0_3, A0_3.TEXT_CLSACN149_01103_THUBYRGEIM_000_001, false)
    A2_5:PlayActionTimeline(A0_3.ACTION_TIMELINE_EVENT_TALK2)
    A2_5:Talk(A1_4, A0_3, A0_3.TEXT_CLSACN149_01103_THUBYRGEIM_000_002, false)
    A2_5:Talk(A1_4, A0_3, A0_3.TEXT_CLSACN149_01103_THUBYRGEIM_000_003, true)
    A0_3:QuestAccepted()
  end
  function ClsAcn149.OnScene00002(A0_6, A1_7, A2_8)
    A2_8:TurnTo(A1_7, false)
    A2_8:WaitForTurn()
    A2_8:PlayActionTimeline(A0_6.ACTION_TIMELINE_EVENT_GREETING)
    A2_8:Talk(A1_7, A0_6, A0_6.TEXT_CLSACN149_01103_KLYHIA_000_020, false)
    A2_8:Talk(A1_7, A0_6, A0_6.TEXT_CLSACN149_01103_KLYHIA_000_021, false)
    A2_8:PlayActionTimeline(A0_6.ACTION_TIMELINE_EVENT_ADD_YES)
    A2_8:Talk(A1_7, A0_6, A0_6.TEXT_CLSACN149_01103_KLYHIA_000_022, false)
    A2_8:PlayActionTimeline(A0_6.ACTION_TIMELINE_EVENT_TALK_FOREFINGER)
    A2_8:Talk(A1_7, A0_6, A0_6.TEXT_CLSACN149_01103_KLYHIA_000_023, false)
    while true do
      if A0_6:Menu(A0_6.TEXT_CLSACN149_01103_Q1_000_000, A0_6.TEXT_CLSACN149_01103_A1_000_000, A0_6.TEXT_CLSACN149_01103_A2_000_002) == 1 then
        A2_8:PlayActionTimeline(A0_6.ACTION_TIMELINE_EMOTE_JOY)
        A2_8:Talk(A1_7, A0_6, A0_6.TEXT_CLSACN149_01103_KLYHIA_000_030, false)
        A2_8:Talk(A1_7, A0_6, A0_6.TEXT_CLSACN149_01103_KLYHIA_000_031, false)
        A2_8:PlayActionTimeline(A0_6.ACTION_TIMELINE_EVENT_ADD_YES)
        A2_8:Talk(A1_7, A0_6, A0_6.TEXT_CLSACN149_01103_KLYHIA_000_032, true)
        break
      else
        A2_8:PlayActionTimeline(A0_6.ACTION_TIMELINE_EVENT_ADD_NO)
        A2_8:Talk(A1_7, A0_6, A0_6.TEXT_CLSACN149_01103_KLYHIA_000_025, true)
      end
    end
  end
  function ClsAcn149.OnScene00003(A0_9, A1_10, A2_11)
    local L3_12, L4_13
    L4_13 = A2_11
    L3_12 = A2_11.TurnTo
    L3_12(L4_13, A1_10, false)
    L4_13 = A2_11
    L3_12 = A2_11.Talk
    L3_12(L4_13, A1_10, A0_9, A0_9.TEXT_CLSACN149_01103_THUBYRGEIM_000_040, false)
    L4_13 = A2_11
    L3_12 = A2_11.PlayActionTimeline
    L3_12(L4_13, A0_9.ACTION_TIMELINE_EVENT_TALK2)
    L4_13 = A2_11
    L3_12 = A2_11.Talk
    L3_12(L4_13, A1_10, A0_9, A0_9.TEXT_CLSACN149_01103_THUBYRGEIM_000_041, true)
    L4_13 = A0_9
    L3_12 = A0_9.QuestReward
    L4_13 = L3_12(L4_13, A2_11, A1_10)
    if L3_12 then
      A0_9:QuestCompleted()
      A0_9:Wait(120)
    end
    return L3_12, L4_13
  end
  function ClsAcn149.OnScene00004(A0_14, A1_15, A2_16, ...)
    local L4_18
    L4_18 = (...)
    A1_15:LookAt()
    A2_16:Visible(A0_14.VISIBLE_HIDE)
    A0_14:PlayCamera(6, A1_15)
    A0_14:FollowLookAt(A0_14.FOLLOW_LOOKAT_ON)
    A0_14:Zoom(-1, -1, 0, 0, 0)
    A0_14:UpdownDolly(0.4, 0.4, 0, 0, 0)
    A0_14:Gyro(-20, -20, 0, 0, 0)
    A0_14:LearningAction(A0_14.ACTION_KIND_NORMAL, A0_14.LOC_WS)
    A0_14:Wait(60)
    A1_15:PlayActionTimeline(A0_14.LOC_ACTION0_WSGET, nil, A0_14.AUTO_SHAKE_ENABLE, A0_14.ACTION_NO_INTERPOLATE)
    A0_14:FadeIn(A0_14.FADE_SHORT)
    A0_14:WaitForFade()
    A1_15:PlayVfx(A0_14.LOC_VFX)
    A0_14:Wait(100)
    A0_14:FadeOut(A0_14.FADE_DEFAULT)
    A0_14:WaitForFade()
    A1_15:CancelActionTimeline(A0_14.LOC_ACTION0_WSGET)
    A0_14:Wait(30)
    A1_15:LookAt()
    A2_16:LookAt()
    return L4_18
  end
  function ClsAcn149.OnScene00005(A0_19, A1_20, A2_21)
    A2_21:LookAt(A1_20)
    A2_21:Talk(A1_20, A0_19, A0_19.TEXT_CLSACN149_01103_KLYHIA_000_035, true)
  end
  function ClsAcn149.IsTodoChecked(A0_22, A1_23, A2_24)
    local L3_25
    L3_25 = A0_22.GetQuestId
    L3_25 = L3_25(A0_22)
    if A1_23:GetQuestSequence(L3_25) == A0_22.SEQ_0 then
      return false
    end
    if A2_24 == 0 then
      return A1_23:GetQuestUI8AL(L3_25) >= 1
    elseif A2_24 == 1 then
      return false
    end
  end
end)()
;(function()
  local L0_26, L1_27
  L0_26 = ClsAcn149
  L0_26.SCRIPT_VERSION = 1
  L0_26 = ClsAcn149
  function L1_27(A0_28)
    local L1_29
  end
  L0_26.OnInitialize = L1_27
  L0_26 = ClsAcn149
  function L1_27(A0_30, A1_31, A2_32, A3_33, A4_34)
    local L5_35
    L5_35 = A0_30.GetQuestId
    L5_35 = L5_35(A0_30)
    if A1_31:GetQuestSequence(L5_35) == A0_30.SEQ_FINISH then
      if A3_33 == A0_30.ACTOR0 then
        return true
      elseif A3_33 == A0_30.ACTOR1 then
        return true
      end
    end
    return false
  end
  L0_26.IsAcceptEvent = L1_27
  L0_26 = ClsAcn149
  function L1_27(A0_36, A1_37, A2_38, A3_39, A4_40)
    local L5_41
    L5_41 = A0_36.GetQuestId
    L5_41 = L5_41(A0_36)
    if A1_37:GetQuestSequence(L5_41) == A0_36.SEQ_FINISH then
      if A3_39 == A0_36.ACTOR0 then
        return true
      elseif A3_39 == A0_36.ACTOR1 then
        return false
      end
    end
    return false
  end
  L0_26.IsAnnounce = L1_27
  L0_26 = ClsAcn149
  function L1_27(A0_42, A1_43, A2_44)
    local L3_45
    L3_45 = A0_42.GetQuestId
    L3_45 = L3_45(A0_42)
    if A1_43:GetQuestSequence(L3_45) == A0_42.SEQ_0 then
      return 0, 0
    end
    if A2_44 == 0 then
      return A1_43:GetQuestUI8AL(L3_45), 0
    elseif A2_44 == 1 then
      return A1_43:GetQuestUI8AL(L3_45), 0
    end
  end
  L0_26.GetTodoArgs = L1_27
  L0_26 = ClsAcn149
  function L1_27(A0_46, A1_47, A2_48)
    local L3_49
    L3_49 = A0_46.GetQuestId
    L3_49 = L3_49(A0_46)
    if A1_47:GetQuestSequence(L3_49) == A0_46.SEQ_1 then
    elseif A1_47:GetQuestSequence(L3_49) == A0_46.SEQ_FINISH then
    end
    return A0_46:IsBattleNpcTriggerOwner(A1_47, A2_48, false), false
  end
  L0_26.GetGimmickState = L1_27
end)()
