...
/**
 * php bin/console app:parse:csv Pokemon:Pokemon_Hero_Temp
 */
class Pokemon_Hero_Temp implements ParseInterface
{
    ...
    public function parse()
    {
        ...
        $Pokemon_Base = $this->json("/$Version/Pokemon_Base"); //done
        $OutSideItem_Base = $this->json("/$Version/OutSideItem_Base"); // done
        $Pokemon_Hero = $this->json("/$Version/Pokemon_Hero"); // done
        $Talent_PlanRaw = $this->json("/$Version/Talent_Plan"); // done
        ...
        $Pokemon_Talent = $this->json("/$Version/Pokemon_Talent");// done
        $Pokemon_Hero_Evolution = $this->json("/$Version/Pokemon_Hero_Evolution"); // done
        //$Active_Skill_Hero_Old = $this->json("/$Version/Active_Skill_Hero");
        $Active_Skill_Hero = $this->json("/$Version/Active_Skill_Hero"); // needs work
        ...
        foreach ($Pokemon_Base as $id => $Base) {
            ...
            foreach($PassiveIDGroup as $PassiveID){
                if (!empty($Passive_skill[$PassiveID])){

                    foreach($Passive_skill[$PassiveID]['RefEffectGroupIds'] as $RefRaw){
                        $RefEffect = $RefRaw;
            ...
            $PassiveSkillId = $PassiveIDGroup[0];
            $IconArray[] = $PassiveIconPath;
            $PokemonMoveList[0]["PassiveName"] = $PassiveSkillName;
            ...
            $SkillString .= "|Cooldown = ".$PassiveSkillCD."\n";
